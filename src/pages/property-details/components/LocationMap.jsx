...
const LocationMap = ({ name, locationMap }) => {
  const [scale, setScale] = useState(1);
  const [isOpen, setIsOpen] = useState(false);

  const src = useMemo(() => {
    if (!locationMap) return null;
    if (typeof locationMap === "string") return locationMap;
    return locationMap?.s3_link ?? locationMap?.name ?? null;
  }, [locationMap]);
